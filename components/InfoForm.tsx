  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: existingOrganization, error: checkError } = await supabase
        .from("organizations")
        .select('*')
        .eq('"contactEmail"', customerData.contactEmail)  // Enforce exact case using double quotes
        .single();

      if (checkError) {
        if (checkError.code === "PGRST116") { // Handle "no rows found" if needed
          console.warn("No existing organization found with this email.");
        } else {
          throw checkError;
        }
      }
      ...
      console.error("Submission error:", (error as Error).message || error);
